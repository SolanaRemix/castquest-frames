...
ROOT_DIR="castquest-pr8"
rm -rf "$ROOT_DIR"
mkdir -p "$ROOT_DIR"

# infra/openapi
mkdir -p "$ROOT_DIR/infra/openapi"
cat > "$ROOT_DIR/infra/openapi/castquest-full-openapi.yaml" <<'EOF'
openapi: "3.0.3"
info:
...
paths:
  ...
  /frames/templates/{templateId}/deploy:
    post:
      tags: [frames]
      summary: Deploy a template version (async) — emits frame.deploy.request
      security:
        - bearerAuth: []
      parameters:
        - name: templateId
...
# infra/openapi README
cat > "$ROOT_DIR/infra/openapi/README.md" <<'EOF'
...
# packages/sdk
mkdir -p "$ROOT_DIR/packages/sdk/src/generated"
mkdir -p "$ROOT_DIR/packages/sdk/src/examples"

cat > "$ROOT_DIR/packages/sdk/package.json" <<'EOF'
{
...
cat > "$ROOT_DIR/packages/sdk/tsconfig.json" <<'EOF'
{
  "compilerOptions": {
    "target": "ES2019",
    "module": "CommonJS",
...
cat > "$ROOT_DIR/packages/sdk/src/generated/types.ts" <<'EOF'
...
cat > "$ROOT_DIR/packages/sdk/src/index.ts" <<'EOF'
...
cat > "$ROOT_DIR/packages/sdk/src/examples/frame-builder.ts" <<'EOF'
/**
...
cat > "$ROOT_DIR/packages/sdk/README.md" <<'EOF'
...
# infra/db/migrations
mkdir -p "$ROOT_DIR/infra/db/migrations"

cat > "$ROOT_DIR/infra/db/migrations/0001_create_tenants_users.sql" <<'EOF'
...
cat > "$ROOT_DIR/infra/db/migrations/0002_auth_sessions_apikeys.sql" <<'EOF'
...
cat > "$ROOT_DIR/infra/db/migrations/0003_projects_files.sql" <<'EOF'
...
cat > "$ROOT_DIR/infra/db/migrations/0004_previews_agents_shipments.sql" <<'EOF'
...
cat > "$ROOT_DIR/infra/db/migrations/0005_frames_templates.sql" <<'EOF'
...
cat > "$ROOT_DIR/infra/db/migrations/0006_ai_jobs_tailors.sql" <<'EOF'
...
cat > "$ROOT_DIR/infra/db/migrations/0007_market_mints_ledger.sql" <<'EOF'
...
cat > "$ROOT_DIR/infra/db/migrations/0008_quests_achievements_leaderboard.sql" <<'EOF'
...
cat > "$ROOT_DIR/infra/db/migrations/0009_protocol_audit_system.sql" <<'EOF'
...
cat > "$ROOT_DIR/infra/db/migrations/0010_social_wallets.sql" <<'EOF'
...
cat > "$ROOT_DIR/infra/db/migrations/README.md" <<'EOF'
# DB Migrations
...
mkdir -p "$ROOT_DIR/infra/.github/workflows"
cat > "$ROOT_DIR/infra/.github/workflows/openapi-sdk.yml" <<'EOF'
...
# PR description
cat > "$ROOT_DIR/PR_DESCRIPTION.md" <<'EOF'
...
# Create the zip
rm -f castquest-pr8.zip
(cd "$ROOT_DIR" && zip -r ../castquest-pr8.zip .)

echo "Created castquest-pr8.zip in $(pwd)"
echo "Directory '$ROOT_DIR' contains the generated files. Review before committing."
EOF